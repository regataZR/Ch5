...
void recondResponse(int i, int response);
void highestRatings();
void lowestRatings();
...
int main(void)
{
	...
	do
	{
		...
		for (i = 0; i< ISSUES; i++)
		{
			...
			recondResponse(i, response);	

		}
		printf("Enter 1 to stop. Enter 0 to rate the issues againn. ");
		scanf_s("%d", &response);
	}while (response != 1);
...
void recondResponse(int issue, int rating)
{
...
void highestRatings(void)
{
	...
	printf(" wish a total rating of %d\n", highRating);
}
